...
def generate_stim_circuit(rounds=10000, noise_level=0.01):
...
def run_sparse_loop(model_path='fridge.pt'):
    device = torch.device("cpu")
    print(f"--- The Fridge: Sparse Training Mode (Device: {device}) ---")
    ...
    print("\n--- LIVE STREAM STARTED ---")
    print("Logic: If Surprise > 0.05 -> TRAIN. Else -> INFERENCE.\n")
    ...
    try:
        while True:
            ...
            for i in range(full_stream.shape[0] - 1):
                ...
                # 2. Calculate Surprise (No Backprop yet)
                # Using MSE as the "Attention Trigger"
                loss = nn.MSELoss()(prediction, future_syndrome)
                surprise = loss.item()
                
                # 3. THE GATE: Sparse Training Logic
                # Train if surprised OR every 1000th event (heartbeat)
                should_train = (surprise > 0.05) or (total_events % 1000 == 0)
                ...
                if total_events % 1000 == 0:
                    latency_ns = (time.time_ns() - t0) / 1000
                    acc = (total_correct_bits / total_bits) * 100
                    training_rate = (trained_events / 1000) * 100
                    ...
                    print(f"Event #{total_events:,} | Latency: {latency_ns/1000:.2f} µs | Accuracy: {acc:.2f}% | Training Rate: {training_rate:.1f}% [{status}]")

    except KeyboardInterrupt:
        print("\nSaving sparse-optimized brain...")
        torch.save(fridge.state_dict(), 'fridge_sparse.pt')
